...
func (gc *GarbageCollector) Run(ctx context.Context, tree *statetree.Tree, reason TriggerReason) GCResult {
	result := GCResult{Triggered: true, Reason: reason}

	activeNodes := tree.GetActiveNodes()
	currentTurn := tree.TurnCount()

	gc.scorer.ScoreAll(activeNodes, currentTurn)
	...
	for _, node := range activeNodes {
		if node.Scores.RetentionScore < gc.policy.LowRelevanceThreshold {
...
func (gc *GarbageCollector) ForceRun(ctx context.Context, tree *statetree.Tree) GCResult {
	return gc.Run(ctx, tree, ReasonManual)
}
...
func (c *SimpleCompressor) Compress(_ context.Context, nodes []*domain.StateNode) (*domain.StateNode, error) {
	var decisions, issues []string
	var eventRefs []string
	var summaryParts []string

	for _, n := range nodes {
		decisions = append(decisions, n.Decisions...)
	...
	summary := "Compressed branch: "
	if len(summaryParts) > 3 {
		summary += summaryParts[0] + " ... " + summaryParts[len(summaryParts)-1]
	} else {
